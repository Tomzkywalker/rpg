...
export const useGameStore = defineStore('game', () => {
  ...
  function calculatePlayerDamage() {
    ...
    const isCritical = Math.random() < criticalChance.value / 100

    const rawDamage = attack.value + randomBonus

    return {
      damage: Math.max(
        damageConfig.min,

        Math.round(rawDamage * (isCritical ? COMBAT_CONFIG.criticalDamageMultiplier : 1)),
      ),
